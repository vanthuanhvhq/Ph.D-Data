...
#define RPLIDAR_MOTOR 10                          // For motor of the Lidar                           
...
float omega_FLC_R, omega_FLC_L, dT;
...
void setup()
{
  ...
  pinMode(RPLIDAR_MOTOR, OUTPUT);                // Set pin 10 as OUTput (Pin 10 is connected to MOTOCTL pin)
  ...
  FuzzySet *VeryClose = new FuzzySet(0, 0, 0, 2.57475679);      // Instantiating a FuzzySet object
  distance->addFuzzySet(VeryClose);                             // Including the FuzzySet into FuzzyInput
  FuzzySet *Close = new FuzzySet(0.9677053499999999, 2.50155673, 2.50155673, 3.7661125799999997);           // Instantiating a FuzzySet object
  distance->addFuzzySet(Close);                                 // Including the FuzzySet into FuzzyInput
  FuzzySet *Medium = new FuzzySet(2.2327540699999995, 4.10836493, 4.10836493, 6.0118431);        // Instantiating a FuzzySet object
  distance->addFuzzySet(Medium);                                // Including the FuzzySet into FuzzyInput
  FuzzySet *Far = new FuzzySet(6.61510885, 8.94806436, 8.94806436, 16.23207958);             // Instantiating a FuzzySet object
  distance->addFuzzySet(Far);                                   // Including the FuzzySet into FuzzyInput
  FuzzySet *VeryFar = new FuzzySet(5.49427774, 10, 10, 10);              // Instantiating a FuzzySet object
  ...
  FuzzySet *BigNegative = new FuzzySet(-3.14, -3.14, -3.14, -1.9399301700000002);   // Instantiating a FuzzySet object
  deltaTheta->addFuzzySet(BigNegative);                         // Including the FuzzySet into FuzzyInput
  FuzzySet *Negative = new FuzzySet(-1.761151646, -1.46725023, -1.46725023, 0.6507242099999997);      // Instantiating a FuzzySet object
  deltaTheta->addFuzzySet(Negative);                            // Including the FuzzySet into FuzzyInput
  FuzzySet *Zero = new FuzzySet(-2.88213399, -0.92990968, -0.92990968, -0.8953723319);             // Instantiating a FuzzySet object
  deltaTheta->addFuzzySet(Zero);                                // Including the FuzzySet into FuzzyInput
  FuzzySet *Positive = new FuzzySet(-0.8851117500000001, 2.18321422, 2.18321422, 2.551341773);         // Instantiating a FuzzySet object
  deltaTheta->addFuzzySet(Positive);                            // Including the FuzzySet into FuzzyInput
  FuzzySet *BigPositive = new FuzzySet(1.9908585200000002, 3.14, 3.14, 3.14);       // Instantiating a FuzzySet object
  ...
  FuzzySet *VerySmallRight = new FuzzySet(0, 0, 0, 13.2435929);        // Instantiating a FuzzySet object
  omegaR->addFuzzySet(VerySmallRight);                          // Including the FuzzySet into FuzzyOutput
  FuzzySet *SmallRight = new FuzzySet(-7.899351950000001, 4.99362005, 4.99362005, 7.566785019999999);         // Instantiating a FuzzySet object
  omegaR->addFuzzySet(SmallRight);                              // Including the FuzzySet into FuzzyOutput
  FuzzySet *MediumBigRight = new FuzzySet(13.0838108569, 13.1089719, 13.1089719, 13.11355919417);   // Instantiating a FuzzySet object
  omegaR->addFuzzySet(MediumBigRight);                          // Including the FuzzySet into FuzzyOutput
  FuzzySet *BigRight = new FuzzySet(7.60264836, 17.1017795, 17.1017795, 17.113000963799998);        // Instantiating a FuzzySet object
  omegaR->addFuzzySet(BigRight);                                // Including the FuzzySet into FuzzyOutput
  FuzzySet *VeryBigRight = new FuzzySet(22.08156513, 30, 30, 30);      // Instantiating a FuzzySet object
  ...
  FuzzySet *VerySmallLeft = new FuzzySet(0, 0, 0, 11.9429052);         // Instantiating a FuzzySet object
  omegaL->addFuzzySet(VerySmallLeft);                           // Including the FuzzySet into FuzzyOutput
  FuzzySet *SmallLeft = new FuzzySet(-6.3058226159999995, 0.018213374, 0.018213374, 8.954687494);          // Instantiating a FuzzySet object
  omegaL->addFuzzySet(SmallLeft);                               // Including the FuzzySet into FuzzyOutput
  FuzzySet *MediumBigLeft = new FuzzySet(11.61275869, 13.9808695, 13.9808695, 17.94023197);    // Instantiating a FuzzySet object
  omegaL->addFuzzySet(MediumBigLeft);                           // Including the FuzzySet into FuzzyOutput
  FuzzySet *BigLeft = new FuzzySet(7.689378100000001, 17.1023131, 17.1023131, 17.315006372);         // Instantiating a FuzzySet object
  omegaL->addFuzzySet(BigLeft);                                 // Including the FuzzySet into FuzzyOutput
  FuzzySet *VeryBigLeft = new FuzzySet(15.2590854, 30, 30, 30);       // Instantiating a FuzzySet object
...
void loop()
{
  ...
  {
    if (results.value == 0xFFE21D) // Start run robot (CH+ button)
    {
      //------------------------Run motor--------------------------------------
    ...
    else if (results.value == 0xFFC23D) // Stop the robot (Stop button)
    {
  ...
  {
    float distance_LD = lidar.getCurrentPoint().distance; // Distance value in mm unit
    float angle_LD = lidar.getCurrentPoint().angle; // Angle value in degree
    bool startBit = lidar.getCurrentPoint().startBit; // Whether this point is belong to a new scan
    byte quality = lidar.getCurrentPoint().quality; // Quality of the current measurement

    distance_LD = distance_LD / 1000; //Convert mm to m
    angle_LD = 180 - angle_LD;          // Correct robot orientation
    angle_LD = (angle_LD * PI) / 180; // convert degree to radian

    if (((0 < distance_LD) && (distance_LD < 4)) && (( -PI / 2 < angle_LD) && ( angle_LD < PI / 2)))
    {
      if (0.5 < distance_LD)
      {
        fuzzy->setInput(1, distance_LD);                   // Set the distance value as the input1
        fuzzy->setInput(2, angle_LD);                      // Set the deltaTheta value as the input2
        ...
        if ((currentMillis_Right - previousMillis_Right > interval) && (currentMillis_Left - previousMillis_Left > interval))
        {
          ...
          Current_Velocity_Odo = (0.0425 * (Omega_Right + Omega_Left)) / 2;   //Calculate linear velocity of robot
          encoderValue_Left = 0;
        ...
        float PWMr_LD = map(omega_PID_R, 0, 30, 0, 255);
        float PWMl_LD = map(omega_PID_L, 0, 30, 0, 255);
        analogWrite(ENA_MOTOR_RIGHT, PWMr_LD); // For right wheel
        analogWrite(ENA_MOTOR_LEFT, PWMl_LD);  // For left wheel
    ...
    }// For if (((0 < distance_LD) && (distance_LD < 4)) && (( -PI / 25 < angle_LD) && ( angle_LD < PI / 25)))
  }// cua if Lidar
  else {
